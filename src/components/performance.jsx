  ...
  return (
    <div className="bg-white p-4 rounded-xl border border-[#D0D5DD] flex flex-col gap-5 items-start justify-start">
      <div className='flex justify-between items-center'>
        <h3 className="text-[15px] max-w[100px] font-semibold text-gray-900">Total view performance</h3>

      {/* Legend at the top right, as per image */}
      <div className="w-[115px]"> {/* Adjusted margin to align with top-right of chart area */}
        <ul className="text-[10px] text-gray-600 flex flex-col gap-[6px]">
          {data.map((entry, index) => (
            <li key={`legend-${index}`} className="flex items-center gap-[1px]">
      ...
      <div className="relative w-full h-48 flex items-center justify-center"> {/* Adjusted height */}
        ...
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-center flex flex-col items-center">
            ...
            <span className="text-[22px] font-bold text-gray-900 leading-none">{totalCount}</span>
        </div>
      </div>
    </div>
  );
